...
steps/make_mfcc.sh --mfcc-config conf/mfcc.conf --nj 1 --cmd "$train_cmd" \
  $enrolldir exp/make_mfcc $mfccdir
utils/fix_data_dir.sh $enrolldir

sid/compute_vad_decision.sh --nj 1 --cmd "$train_cmd" \
  $enrolldir exp/make_vad $vaddir
...
run.pl $scores_dir/log/plda_scoring.log \
   ...
   "ark:ivector-normalize-length scp:${enroll_ivec_dir}/spk_ivector.scp ark:- | ivector-subtract-global-mean ${plda_ivec_dir}/mean.vec ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
   "cat '$trials' | cut -d\  --fields=1,2 |" $scores_dir/plda_scores || exit 1;    

fi

#结果输出文件
output_file="scores/results"
...
if [ $stage -lt 5 ]; then
echo target scores: | tee -a $output_file
cat scores/plda_scores | awk '$1==$2' | awk 'BEGIN {min=99999}{if ($3+0 < min+0) min=$3} END {print "min: ", min}' | tee -a $output_file
cat scores/plda_scores | awk '$1==$2' | awk 'BEGIN {max=-99999}{if ($3+0 > max+0) max=$3} END {print "max: ", max}' | tee -a $output_file
echo nontarget scores: | tee -a $output_file
cat scores/plda_scores | awk '$1!=$2' | awk 'BEGIN {min=99999}{if ($3+0 < min+0) min=$3} END {print "min: ", min}' | tee -a $output_file
cat scores/plda_scores | awk '$1!=$2' | awk 'BEGIN {max=-99999}{if ($3+0 > max+0) max=$3} END {print "max: ", max}' | tee -a $output_file
